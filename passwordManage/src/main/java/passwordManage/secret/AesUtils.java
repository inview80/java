...
public class AesUtils extends Aes256Abstract {
    ...
    public static AesUtils newInstance(String IV) {
        if(fragment==null){
     ...
     */
    @Override
    public  byte[] encrypt(byte[] content, byte[] pkey) {
        try {
     ...
     */
    @Override
    public  byte[] decode(byte[] content, byte[] pkey) throws Exception {
        SecretKeySpec skey = new SecretKeySpec(tohash256Deal(pkey), "AES");
    ...
    private  byte[] tohash256Deal(byte[] keyBytes) {
        try {
